...
//   LOBI  : buat ruang (master), gabung dengan kode, ruang saya, undangan.
...
import { Copy, Crown, Dice5, DoorOpen, Loader2, Play, Plus, RefreshCw, Search, Trophy, UserPlus, Users, X } from "lucide-react";
import { GlassCard } from "@/components/glass-card";
import { AvatarInisial, GlassSkeleton, ScreenHeader, StatusBadge } from "@/components/pri-ui";
import { FotoBulat } from "@/components/foto-bulat";
import { toast, useAppStore } from "@/hooks/use-app-store";
import { cariKewajiban, getLudoDaftar, getLudoRuang, ludoAksi, type PenggunaKewajiban, type RuangLudo } from "@/services";
...
function KartuPemain({ p, giliran, sisaDetik, rumah, saya }: { p: Pemain; giliran: boolean; sisaDetik: number | null; rumah: number; saya: boolean }) {
...
export function LudoScreen({ onKembali }: { onKembali: () => void }) {
  ...
  const uid = user?.id ?? "";
  const [lobi, setLobi] = useState<{ boleh_buat: boolean; daftar: RuangLudo[] } | null>(null);
  const [ruang, setRuang] = useState<RuangLudo | null>(null);
  const [kode, setKode] = useState("");
  const [cari, setCari] = useState("");
  const [hasilCari, setHasilCari] = useState<PenggunaKewajiban[]>([]);
  ...
  const muatLobi = useCallback(() => {
    return getLudoDaftar()
      .then(setLobi)
      .catch((e) => toast("error", "Gagal memuat lobi", e instanceof Error ? e.message : ""));
  }, []);
  ...
  useEffect(() => {
    ...
    const tik = () => setDetik(Math.max(0, Math.ceil((batas - Date.now()) / 1000)));
    tik();
    const t = setInterval(tik, 1000);
    return () => clearInterval(t);
  }, [ruang?.state?.batas, ruang?.status]);

  // Cari pemain untuk diundang (host = master → boleh memakai pencarian pengguna).
  useEffect(() => {
    if (!ruang?.saya_host || cari.trim().length < 2) return;
    let hidup = true;
    const t = setTimeout(() => {
      cariKewajiban(cari.trim())
        .then((d) => hidup && setHasilCari(d.hasil))
  ...
  async function jalankan(kunci: string, aksi: string, data: Record<string, unknown> = {}, pesan?: string) {
    ...
    try {
      ...
      toast(teks.includes("berubah") ? "info" : "peringatan", teks.includes("berubah") ? "Papan diperbarui" : "Tidak bisa", teks);
      // Konflik versi → ambil keadaan terbaru.
      if (ruangRef.current) getLudoRuang(ruangRef.current.id).then(setRuang).catch(() => {});
      return null;
    } finally {
      setSibuk("");
    }
  ...
  if (!ruang) {
    return (
      <div className="kolom-aplikasi px-4 pb-32">
        ...
        <p className="mb-3 text-[11px] text-teks-sekunder">Modul percobaan · robot pet tiap pemain jadi bidaknya · 2–4 pemain.</p>
        {lobi === null ? (
          ...
          <>
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              {lobi.boleh_buat ? (
                <button
                  type="button"
                  onClick={() => void jalankan("buat", "buat", {}, "Ruang dibuat — undang temanmu!").then((r) => r && setRuang(r))}
                ...
                >
                  {sibuk === "buat" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-5 w-5" />} Buat ruang baru
                </button>
              ) : null}
              <div className="glass flex h-14 items-center gap-2 rounded-2xl px-3">
                ...
                <button
                  type="button"
                  onClick={() => void jalankan("gabung", "gabung", { kode }, "Bergabung!").then((r) => r && setRuang(r))}
            ...
            <GlassCard className="mt-3 p-4">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => void muatLobi()} aria-label="Segarkan" className="btn-tekan p-1 text-teks-sekunder">
                  <RefreshCw className="h-4 w-4" />
                </button>
              </div>
              {lobi.daftar.length === 0 ? (
                <p className="mt-2 text-[11.5px] text-teks-sekunder">Belum ada ruang. {lobi.boleh_buat ? "Buat ruang lalu undang pemain lain." : "Tunggu undangan atau masukkan kode ruang."}</p>
              ) : (
                  ...
                  {lobi.daftar.map((r) => {
                    ...
                    return (
                      <div key={r.id} className="glass-soft flex items-center gap-2 rounded-xl px-3 py-2">
                        <span className="font-mono text-[13px] font-extrabold tracking-widest text-teks-utama">{r.kode}</span>
                        <span className="min-w-0 flex-1 truncate text-[11px] text-teks-sekunder">
                          {r.pemain.map((p) => p.nama.split(" ")[0]).join(", ")} · {r.pemain.length} pemain
                        </span>
                        <StatusBadge label={r.status === "berjalan" ? "berjalan" : r.status === "selesai" ? "selesai" : diundang ? "diundang" : "menunggu"} warna={r.status === "berjalan" ? "hijau" : r.status === "selesai" ? "netral" : "kuning"} />
                        <button
                          type="button"
                          onClick={() => {
                            if (diundang && r.status === "menunggu") void jalankan(`gabung:${r.id}`, "gabung", { kode: r.kode }, "Bergabung!").then((x) => x && setRuang(x));
                            else setRuang(r);
                          }}
                          disabled={Boolean(sibuk)}
                          ...
                          style={{ background: MERAH }}
                        >
                          {diundang && r.status === "menunggu" ? "Gabung" : "Buka"}
                        </button>
                      </div>
                    );
                  })}
            ...
            <GlassCard className="mt-3 p-4">
              <p className="text-[12.5px] font-bold text-teks-utama">Aturan singkat</p>
              <ul className="mt-1 list-disc space-y-0.5 pl-4 text-[11px] leading-relaxed text-teks-sekunder">
                <li>Keluar markas butuh dadu 6. Dadu 6, memakan lawan, atau sampai rumah = giliran lagi (tiga kali 6 = hangus).</li>
                <li>Petak berbintang dan petak awal aman dari tangkapan. Masuk rumah harus pas.</li>
                <li>Giliran maksimal 60 detik; lewat itu server menjalankan langkah otomatis.</li>
              </ul>
            </GlassCard>
          </>
        )}
    ...
    return (
      <div className="kolom-aplikasi px-4 pb-32">
        ...
        <GlassCard className="p-4 text-center">
          <p className="text-[11px] font-bold tracking-wide text-teks-sekunder uppercase">Kode ruang</p>
          <button type="button" onClick={() => void salinKode(ruang.kode)} className="btn-tekan mt-1 inline-flex items-center gap-2 font-mono text-[28px] font-extrabold tracking-[0.3em] text-teks-utama">
            {ruang.kode} <Copy className="h-4 w-4 text-teks-sekunder" />
          </button>
          <p className="text-[11px] text-teks-sekunder">Bagikan kode ini, atau undang lewat nama di bawah.</p>
        </GlassCard>

        <GlassCard className="mt-3 p-4">
          <p className="text-[12.5px] font-bold text-teks-utama">Pemain ({ruang.pemain.length}/4)</p>
          <div className="mt-2 grid grid-cols-2 gap-2">
            ...
            {Array.from({ length: Math.max(0, 4 - ruang.pemain.length) }).map((_, i) => (
              <div key={`kosong-${i}`} className="flex min-h-[110px] flex-col items-center justify-center rounded-xl border-2 border-dashed border-teks-sekunder/30 text-[11px] text-teks-sekunder">
                <UserPlus className="mb-1 h-5 w-5" /> menunggu pemain
              </div>
            ))}
          </div>
          {ruang.undangan.length > 0 ? (
            <p className="mt-2 text-[11px] text-teks-sekunder">Diundang, belum gabung: {ruang.undangan.map((u) => u.nama).join(", ")}</p>
          ) : null}
        </GlassCard>

        {ruang.saya_host ? (
          <GlassCard className="mt-3 p-4">
            <p className="text-[12.5px] font-bold text-teks-utama">Undang pemain</p>
            <div className="glass-input mt-2 flex h-10 items-center gap-2 rounded-xl px-3">
              <Search className="h-4 w-4 text-teks-sekunder" />
              <input value={cari} onChange={(e) => setCari(e.target.value)} placeholder="Cari nama anggota…" className="h-full min-w-0 flex-1 bg-transparent text-[12.5px] text-teks-utama outline-none" />
            </div>
            {cari.trim().length >= 2 ? (
              <div className="mt-2 flex flex-col gap-1">
                {hasilCari.length === 0 ? (
                  <p className="px-1 text-[11px] text-teks-sekunder">Tidak ada anggota dengan nama itu.</p>
                ) : (
                  hasilCari.slice(0, 8).map((o) => {
                    const sudah = ruang.pemain.some((p) => String(p.user_id) === o.id) || ruang.undangan.some((u) => u.user_id === o.id);
                    return (
                      <div key={o.id} className="flex items-center gap-2 rounded-lg px-1.5 py-1">
                        {o.avatar_url ? <FotoBulat src={o.avatar_url} ukuran={28} /> : <AvatarInisial nama={o.nama} ukuran={28} />}
                        <span className="min-w-0 flex-1 truncate text-[12px] font-semibold text-teks-utama">{o.nama}</span>
                        <button
                          type="button"
                          onClick={() => void jalankan(`undang:${o.id}`, "undang", { id: ruang.id, user_id: o.id }, `${o.nama} diundang (notifikasi terkirim)`).then((r) => r && setRuang(r))}
                          disabled={Boolean(sibuk) || sudah || ruang.pemain.length >= 4}
        ...
        <div className="mt-3 flex gap-2">
          {ruang.saya_host ? (
            <>
              <button
                type="button"
                onClick={() => void jalankan("mulai", "mulai", { id: ruang.id }, "Permainan dimulai!").then((r) => r && setRuang(r))}
                ...
                style={{ background: "linear-gradient(135deg, #10B981, #059669)" }}
              >
                {sibuk === "mulai" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />} Mulai ({ruang.pemain.length} pemain)
              </button>
              <button
                type="button"
                onClick={() => {
                  if (window.confirm("Batalkan ruang ini?")) void jalankan("batal", "batalkan", { id: ruang.id }, "Ruang dibatalkan").then((r) => r && setRuang(null));
                }}
                disabled={Boolean(sibuk)}
            ...
            <button
              type="button"
              onClick={() => void jalankan("keluar", "keluar", { id: ruang.id }, "Keluar dari ruang").then((r) => r && setRuang(null))}
              disabled={Boolean(sibuk)}
        ...
        <p className="mt-2 text-center text-[10.5px] text-teks-sekunder">{ruang.saya_host ? "Menunggu pemain bergabung… daftar diperbarui otomatis." : "Menunggu host memulai permainan…"}</p>
      </div>
    );
  }
  ...
  const warnaSaya = saya >= 0 ? WARNA[ruang.pemain[saya].warna].utama : "#334155";
    ...
    <div className="kolom-aplikasi px-4 pb-32">
      ...
      <div className="grid grid-cols-2 gap-1.5">
        {ruang.pemain.map((p, j) => (
          <KartuPemain key={p.user_id} p={p} giliran={ruang.status === "berjalan" && st.giliran === j} sisaDetik={detik} rumah={st.token[j]?.filter((x) => x >= POS_RUMAH).length ?? 0} saya={j === saya} />
        ))}
      </div>

      {/* Papan */}
      <div className="mt-3 flex justify-center">
        <PapanLudo pemain={ruang.pemain} state={st} sayaIndeks={saya} onGerak={(t) => void gerak(t)} sibuk={Boolean(sibuk)} />
      </div>

      {/* Dadu + status giliran */}
      <GlassCard className="mt-3 flex items-center gap-3 p-3">
        <Dadu nilai={st.dadu} berputar={berputar} warna={WARNA[giliranPemain?.warna ?? 0].utama} onLempar={() => void lempar()} boleh={giliranSaya && st.fase === "lempar" && !sibuk} />
        <div className="min-w-0 flex-1">
          {ruang.status === "selesai" ? (
            <p className="text-[13px] font-extrabold text-teks-utama">Permainan selesai</p>
          ) : giliranSaya ? (
            <p className="text-[13px] font-extrabold" style={{ color: warnaSaya }}>
              {st.fase === "lempar" ? "Giliran Anda — ketuk dadu!" : "Pilih robot yang mau digerakkan (yang berdenyut)."}
            </p>
          ) : (
            <p className="text-[13px] font-bold text-teks-utama">
              Giliran <span style={{ color: WARNA[giliranPemain?.warna ?? 0].utama }}>{giliranPemain?.nama}</span>
              {st.fase === "pilih" ? " — sedang memilih token" : " — sedang melempar"}
            </p>
          )}
          <div className="mt-1 max-h-16 overflow-y-auto text-[10.5px] leading-snug text-teks-sekunder">
            {[...st.log].slice(-4).reverse().map((l, i) => (
              <p key={i} className={cn(i === 0 && "font-semibold text-teks-utama")}>
                {l}
              </p>
            ))}
          </div>
      ...
      {ruang.status === "berjalan" ? (
        <button
          type="button"
          onClick={() => {
            if (window.confirm("Keluar = menyerah. Yakin?")) void jalankan("keluar", "keluar", { id: ruang.id }).then((r) => r && setRuang(r));
          }}
          disabled={Boolean(sibuk)}
      ...
      <AnimatePresence>
        {ruang.status === "selesai" ? (
          <motion.div key="menang" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[90] flex items-center justify-center bg-black/70 p-5" role="dialog" aria-modal="true" aria-label="Hasil permainan">
            <motion.div initial={{ scale: 0.9, y: 16, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} transition={{ type: "spring", stiffness: 260, damping: 22 }} className="glass-strong w-full max-w-sm rounded-3xl p-5 text-center">
              {pemenang ? (
                <>
                  ...
                  <p className="text-[12px] text-teks-sekunder">
                    bersama {pemenang.robot.nama} ({WARNA[pemenang.warna].nama}){Number(pemenang.user_id) === Number(uid) ? " — selamat, itu Anda! 🎉" : ""}
                  </p>
                </>
              ) : (
                <p className="font-heading text-[18px] font-extrabold text-teks-utama">Permainan berakhir</p>
              )}
              <button type="button" onClick={() => setRuang(null)} className="btn-tekan mt-4 h-11 w-full rounded-xl text-[13px] font-bold text-white" style={{ background: MERAH }}>
                Kembali ke lobi
              </button>
            </motion.div>
          </motion.div>
