...
export async function GET(request: Request) {
  return bungkus(async () => {
    ...
    return {
      cakupan_boleh: cakupanPengumuman(user),
...
export async function POST(request: Request) {
  return bungkus(async () => {
    const user = await pastikanMasuk(request);
    const idKu = Number(user.id);
    const boleh = cakupanPengumuman(user);
    if (boleh.length === 0) {
      throw Object.assign(
        new Error("Anda belum berwenang mengirim pengumuman (butuh jabatan struktur atau tim)."),
        { status: 403 },
      );
    }
    ...
    const cakupan = (body.cakupan ?? "") as "semua" | "jabatan" | "tim";
    ...
    if (penerima.length === 0) {
      throw Object.assign(new Error("Tidak ada penerima untuk cakupan ini."), { status: 400 });
    }
