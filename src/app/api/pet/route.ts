...
const KOLOM =
  "user_id, jenis, nama, kenyang, energi, senang, bersih, tidur, xp, aksesoris_dimiliki, aksesoris_terpasang, sparepart_dimiliki, sparepart_terpasang, makanan, aktivitas_hari_ini, aktivitas_tanggal, hadiah_terakhir, terakhir_dihitung, dibuat_pada";
const SLOT_SAH = new Set<string>(["kepala", "mata", "leher", "badan", "punggung", "tangan", "aura"]);
const BAGIAN_SAH = new Set<string>(["kepala", "mata", "tubuh", "kaki", "tangan"]);
...
function aktivitasHariIni(b: Baris): number {
  return b.aktivitas_tanggal === tanggalWib() ? Number(b.aktivitas_hari_ini ?? 0) : 0;
}
...
async function segarkan(db: ReturnType<typeof supabase>, b: Baris): Promise<Baris> {
  ...
  const k = hitungPenurunan(kebutuhanDari(b), b.tidur, jam, aktivitasHariIni(b));
...
function keState(b: Baris | null, saldo: number, pemilik = "", publik = false): PetState {
  ...
  for (const [bagian, kode] of Object.entries(b.sparepart_terpasang ?? {})) {
    if (BAGIAN_SAH.has(bagian) && sparepartDariKode(String(kode))) sparepartTerpasang[bagian as BagianSparepart] = String(kode);
  }
  const makanan: Record<string, number> = {};
  if (!publik) {
    for (const [kode, n] of Object.entries(b.makanan ?? {})) {
      if (makananDariKode(kode) && Number(n) > 0) makanan[kode] = Math.floor(Number(n));
    }
  }
  return {
    ada: true,
    ...
    dimiliki: publik ? [] : (b.aksesoris_dimiliki ?? []).filter((x) => aksesorisDariKode(x)),
    terpasang,
    sparepart_dimiliki: publik ? [] : (b.sparepart_dimiliki ?? []).filter((x) => sparepartDariKode(x)),
...
export async function GET(request: Request) {
  return bungkus(async () => {
    ...
    if (lihat > 0 && lihat !== Number(user.id)) {
      const [b, { data: orang }] = await Promise.all([bacaBaris(db, lihat), db.from("app_user").select("nama").eq("id", lihat).maybeSingle()]);
...
export async function POST(request: Request) {
  return bungkus(async () => {
    ...
    const balas = async (b: Baris | null, pesan: string) => ({ ...keState(b, await saldoKoin(uid), user.nama), pesan });
    ...
    if (aksi === "ganti_jenis") {
      ...
      return balas({ ...b, jenis }, "Jenis robot diganti — aksesoris, sparepart, dan level tetap.");
    }
    ...
    if (aksi === "tidur" || aksi === "bangun") {
      ...
      return balas({ ...b, tidur, terakhir_dihitung: kini }, tidur ? `${b.nama} tidur… energinya pulih 15 per jam.` : `${b.nama} bangun!`);
    }
    ...
    if (aksi === "makan") {
      ...
      if (sisa <= 0) galat(`${item.nama} tidak ada di inventori — beli dulu di Toko Makanan.`);
      const k = kebutuhanDari(b);
      if (k.kenyang >= 100 && (item.efek.kenyang ?? 0) > 0 && !item.efek.energi && !item.efek.senang) galat(`${b.nama} sudah kenyang.`);
      ...
      const sesudah: Baris = { ...b, ...baru, xp, makanan: inv, aktivitas_hari_ini: aktivitasBaru, aktivitas_tanggal: hariIni, terakhir_dihitung: kini, hadiah_terakhir: dapatHadiah ? hariIni : b.hadiah_terakhir };
      ...
      return balas(
        ...
        `${b.nama} makan ${item.nama} ${item.emoji} (${efek}) · +${XP_MAKAN} XP` +
          (levelDariXp(xp).level > levelDariXp(b.xp).level ? ` · NAIK ke level ${levelDariXp(xp).level}!` : "") +
    ...
    if (aksi === "rawat") {
      ...
      if (b.tidur) galat(`${b.nama} sedang tidur — bangunkan dulu.`);
      const k = kebutuhanDari(b);
      if (jenis === "main" && k.energi < 15) galat(`Energi ${b.nama} kurang untuk bermain — beri minuman berenergi atau biarkan tidur dulu.`);
      ...
      const sesudah: Baris = { ...b, ...baru, xp, aktivitas_hari_ini: aktivitasBaru, aktivitas_tanggal: hariIni, terakhir_dihitung: kini, hadiah_terakhir: dapatHadiah ? hariIni : b.hadiah_terakhir };
      return balas(
        sesudah,
    ...
    if (aksi === "beli") {
      ...
      const item = aks ?? mkn ?? spr;
      if (!item) galat("Barang tidak ada di toko.");
      if (aks && (b.aksesoris_dimiliki ?? []).includes(kode)) galat(`${aks.nama} sudah Anda miliki.`, 409);
      if (spr && (b.sparepart_dimiliki ?? []).includes(kode)) galat(`${spr.nama} sudah Anda miliki.`, 409);
      const saldo = await saldoKoin(uid);
      if (saldo < item.harga) galat(`Koin kurang: butuh ${item.harga}, saldo ${saldo}.`);
      // Buku besar koin: baris negatif. Aksesoris/sparepart unik per kode (anti
      // dobel); makanan boleh berkali-kali → referensi memuat waktu.
      const referensi = mkn ? `${kode}-${Date.now()}` : kode;
      const { error: eKoin } = await db.from("koin_transaksi").insert({ user_id: uid, jumlah: -item.harga, aktivitas: "pet_beli", referensi });
      if (eKoin) {
      ...
      if (aks) {
        ...
        const terpasang = { ...(b.aksesoris_terpasang ?? {}), [aks.slot]: kode };
        kolom = { aksesoris_dimiliki: dimiliki, aksesoris_terpasang: terpasang };
        sesudah = { ...b, aksesoris_dimiliki: dimiliki, aksesoris_terpasang: terpasang };
        pesan = `${aks.nama} dibeli (−${aks.harga} koin) dan langsung dipasang.`;
      } else if (spr) {
        const dimiliki = [...(b.sparepart_dimiliki ?? []), kode];
        const terpasang = { ...(b.sparepart_terpasang ?? {}), [spr.bagian]: kode };
        kolom = { sparepart_dimiliki: dimiliki, sparepart_terpasang: terpasang };
        sesudah = { ...b, sparepart_dimiliki: dimiliki, sparepart_terpasang: terpasang };
      ...
      const { error } = await db.from("pet_robot").update({ ...kolom, diperbarui_pada: kini }).eq("user_id", uid);
    ...
    if (aksi === "pasang") {
      ...
      return balas({ ...b, aksesoris_terpasang: terpasang }, `${item.nama} dipasang.`);
    }
    ...
    if (aksi === "lepas") {
      ...
      await db.from("pet_robot").update({ aksesoris_terpasang: terpasang, diperbarui_pada: kini }).eq("user_id", uid);
      return balas({ ...b, aksesoris_terpasang: terpasang }, "Aksesoris dilepas.");
    }

    if (aksi === "pasang_sparepart") {
      const item = sparepartDariKode(String(body.kode ?? ""));
      if (!item) galat("Sparepart tidak dikenal.");
      if (!(b.sparepart_dimiliki ?? []).includes(item.kode)) galat(`${item.nama} belum dimiliki — beli dulu di Toko Sparepart.`);
      const terpasang = { ...(b.sparepart_terpasang ?? {}), [item.bagian]: item.kode };
      await db.from("pet_robot").update({ sparepart_terpasang: terpasang, diperbarui_pada: kini }).eq("user_id", uid);
      return balas({ ...b, sparepart_terpasang: terpasang }, `${item.nama} dipasang.`);
    }
    ...
    if (aksi === "lepas_sparepart") {
      ...
      await db.from("pet_robot").update({ sparepart_terpasang: terpasang, diperbarui_pada: kini }).eq("user_id", uid);
      return balas({ ...b, sparepart_terpasang: terpasang }, "Kembali ke bagian bawaan.");
    }
